...
data <- data.frame(read_tsv("results_liberal_r2_0.2_eqtlgen_t2dm_risk_all.txt")) # read in data
data$tissue <- "eqtlgen"
...
data$or_ci <- str_c(round(data$or, digits = 3), " (", round(data$or_lci95, digits = 2), ", ",round(data$or_uci95, digits = 2), ")")
...
pdf("plots/forest_t2dm_risk.pdf", width = 20,height = 10, onefile=FALSE)
forestplot(table_text, # columns to include
           graph.pos = 3, # where graph is
           boxsize = 0.2, # size of the box
           ...
           txt_gp = fpTxtGp(summary = list(gpar(cex=2, fontface = "plain")), # text size etc
                            label = gpar(cex=2),
                            ticks = gpar(cex=2),
                            xlab  = gpar(fontface="bold", cex = 2)),
           fn.ci_sum=function(col, size, ...) {fpDrawNormalCI(clr.line = col, clr.marker = col, size=0.2, ...)} # format the "summary" rows to suit the main rows
)
dev.off()
...
data <- data.frame(read_tsv("results_liberal_r2_0.2_eqtlgen_bmi_all.txt"))
data$tissue <- "eqtlgen"
...
pdf("plots/forest_bmi.pdf", width = 20,height = 10, onefile=FALSE)
forestplot(table_text, # columns to include
           graph.pos = 3, # where graph is
           boxsize = 0.2, # size of the box
           align = "l", # align text to left 
...
pdf("plots/forest_pd_risk.pdf", width = 20,height = 10, onefile=FALSE)
forestplot(table_text, # columns to include
           graph.pos = 3, # where graph is
           boxsize = 0.2,
           align = "l",
           ...
           txt_gp = fpTxtGp(summary = list(gpar(cex=2, fontface = "plain")),
                            label = gpar(cex=2),
                            ticks = gpar(cex=2),
                            xlab  = gpar(fontface="bold", cex = 2)),
           fn.ci_sum=function(col, size, ...) {fpDrawNormalCI(clr.line = col, clr.marker = col, size=0.2, ...)}
)
dev.off()
...
pdf("plots/forest_pd_aao.pdf", width = 20,height = 10, onefile=FALSE)
forestplot(table_text, # columns to include
           graph.pos = 3, # where graph is
           boxsize = 0.2,
           align = "l",
...
#table_text <- table_text[complete.cases(table_text),]
table_text <- rbind(c("Gene","Outcome", "Adjusted P"), table_text)


pdf("plots/forest_all.pdf", width = 15,height = 15, onefile=FALSE)
forestplot(table_text, # columns to include
           graph.pos = 3, # where graph is
